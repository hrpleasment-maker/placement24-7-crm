...
export const AdminPortal: React.FC<AdminPortalProps> = ({ onLogout }) => {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'all-leads' | 'telecallers' | 'hr' | 'recruitment' | 'reports' | 'settings'
  >('overview');
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      try {
        ...
        if (tcData?.success && Array.isArray(tcData.telecallers)) {
          setTelecallers(tcData.telecallers);
        }
      } catch {
        console.warn('Could not load telecallers.');
      }
      ...
      try {
        ...
        console.warn('Could not load Google Sheets config.');
      }
    } catch (error) {
  ...
  const handleAddTelecaller = async (e: React.FormEvent) => {
    ...
    const username = newTcData.username.trim().toLowerCase();
    const password = newTcData.password.trim() || 'telecaller123';
    ...
    try {
      ...
      // Read as text first.
      // This prevents JSON parse errors from hiding the real API error.
      ...
      console.log('Telecaller API response:', {
      ...
      if (!data?.success) {
        alert(
          `Failed to add telecaller.\n\n${
            data?.message || 'Server did not confirm creation.'
          }`
        );
        return;
      }

      alert(`Telecaller "${name}" added successfully!`);
    ...
    try {
      const response = await fetch('/api/leads/batch-assign', {
        ...
        body: JSON.stringify({
          ...
          telecallerName: batchAssignTelecaller,
        }),
      });
      ...
      if (!response.ok || !data?.success) {
        alert(data?.message || 'Failed to reassign leads.');
        return;
      }
  ...
  const todayLeads = leads.filter(
    (lead) => lead.date === todayStr
  ).length;

  const interestedLeads = leads.filter(
    (lead) =>
      lead.status === 'Interested' ||
      lead.status === 'Documents Received'
  ).length;

  const followUpPending = leads.filter(
    (lead) =>
      lead.status === 'Follow-up' ||
      lead.status === 'Callback Later' ||
      lead.status === 'Verification Pending'
  ).length;

  const convertedLeads = leads.filter(
    (lead) =>
      lead.status === 'Completed' ||
      lead.status === 'Account Opened' ||
      lead.status === 'Credit Card Approved' ||
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    const matchesTelecaller =
      ...
      lead.assignedTelecaller === selectedTelecallerFilter;
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {[
                ['Total Leads', totalLeads, '100% Captured', 'text-white'],
                ["Today's Leads", todayLeads, 'New Submissions', 'text-amber-400'],
                ['Interested Leads', interestedLeads, 'In Progress', 'text-blue-400'],
                ['Follow-up Pending', followUpPending, 'Scheduled Calls', 'text-purple-400'],
                ['Converted Leads', convertedLeads, 'Sanctioned / Opened', 'text-emerald-400'],
                ['Not Interested', notInterestedLeads, 'Closed / Dropped', 'text-rose-400'],
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col lg:flex-row gap-4">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedTelecallerFilter(e.target.value)
                }
                className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs"
              >
            ...
            <div className="bg-slate-900 border border-amber-500/30 rounded-2xl p-4 flex flex-col sm:flex-row gap-3 justify-between">
              ...
              <div className="flex gap-2">
                <select
                  ...
                  onChange={(e) =>
                    setBatchAssignTelecaller(e.target.value)
                  }
                  className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs"
                >
                  <option value="">
        ...
        {activeTab === 'telecallers' && (
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-3">
              ...
              <button
                onClick={() =>
                  setShowAddTelecallerModal(true)
                }
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {telecallers.map((tc) => {
                  const assigned = leads.filter(
                    (lead) =>
                      lead.assignedTelecaller === tc.name
                  );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90">
          ...
          <div className="bg-slate-900 border border-amber-500/40 rounded-3xl max-w-lg w-full p-6">
            ...
            <div className="bg-slate-950 p-5 rounded-xl mt-4 text-sm leading-7">
              ...
              <p className="mt-3">
                Monthly Compensation: ₹22,000 - ₹28,000 +
